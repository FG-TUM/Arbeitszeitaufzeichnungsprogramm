...
def parse_args() -> argparse.Namespace:
    ...
    vacation_parser.add_argument('days', type=float, choices=[0.5, 1.0], help='Vacation days: 0.5 or 1.0')

    # Sick leave command
    sick_parser = subparsers.add_parser('sick', help='Log sick leave')
    sick_parser.add_argument('days', type=float, choices=[0.5, 1.0], help='Sick leave days: 0.5 or 1.0')

    # Print/show command
    show_parser = subparsers.add_parser('show', help='Show schedule')
    show_parser.add_argument('days', type=int, help='Number of days to show', default=None)
...
def main():
    ...
    command_map = {
        'start': (log_start_time, f"Logged start time {args.time} for {work_date}"),
        'end': (log_end_time, f"Logged end time {args.time} for {work_date}"),
        'break': (log_break_time, f"Logged break time {args.time} for {work_date}"),
        'vacation': (log_vacation, f"Logged {args.time} day vacation for {work_date}"),
        'sick': (log_sick_leave, f"Logged {args.time} day sick leave for {work_date}"),
        'show': (show_csv, ''),
    ...
    try:
        ...
        fun(work_date, args.time, path=config['general']['data_path'])
        show_prev_days()
        if msg:
            print(f"\n{msg}")
